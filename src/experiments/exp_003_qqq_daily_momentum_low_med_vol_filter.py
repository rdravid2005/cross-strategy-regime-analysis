...
Purpose:
Test whether adding a volatility filter improves the simple QQQ daily
momentum strategy from EXP-002.

Strategy rule:
Hold QQQ when trailing 60-day momentum is positive and the prior volatility
regime is low volatility. Otherwise hold cash.
...
class CrossStrategyRegimeAnalysis(QCAlgorithm):
    ...
    def initialize(self):
        ...
        # Baseline parameters chosen before seeing EXP-003 results.
    ...
    def apply_previous_signal_and_filter(self):
        ...
        should_hold_qqq = self.previous_momentum_signal and is_low_volatility
    ...
    def on_end_of_algorithm(self):
        self.debug("EXP-003_QQQ_DAILY_MOMENTUM_LOW_MED_VOL_FILTER")
        self.debug("Strategy: Hold QQQ when 60-day momentum is positive and volatility is low")
        self.debug("Regime timing: today's return uses yesterday's regime label")
        self.debug("Signal timing: today's position uses yesterday's momentum and volatility signals")
        ...
        self.debug(f"Momentum low-volatility exposure: {exposure:.2%}")

        self.debug("MOMENTUM LOW-VOL FILTER STRATEGY OVERALL SUMMARY")
        self.log_summary("all_days", self.daily_records, "strategy_return")

        self.debug("MOMENTUM LOW-VOL FILTER STRATEGY VOLATILITY REGIME SUMMARY")
        self.log_group_summary(classified_records, "volatility_regime", "strategy_return")

        self.debug("MOMENTUM LOW-VOL FILTER STRATEGY TREND REGIME SUMMARY")
        self.log_group_summary(classified_records, "trend_regime", "strategy_return")

        self.debug("MOMENTUM LOW-VOL FILTER STRATEGY COMBINED REGIME SUMMARY")
