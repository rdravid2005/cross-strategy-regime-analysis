...
Purpose:
Test whether the buy-and-hold versus momentum conclusions persist across two
predefined subperiods: 2020-2022 and 2023-2026.

Strategy rule:
The benchmark continuously earns QQQ's close-to-close return. The momentum
strategy holds QQQ only when trailing 60-day momentum is positive and uses an
explicit MarketOnOpen order for the next session.

Timing rule:
All strategy parameters, execution rules, and regime definitions remain fixed.
Subperiods are defined before reviewing their results.
"""
...
class CrossStrategyRegimeAnalysis(QCAlgorithm):
    ...
    def on_end_of_algorithm(self):
        self.debug("EXP-011_QQQ_BH_VS_MOMENTUM_SUBPERIOD_ROBUSTNESS")
        self.debug("Strategies: QQQ buy-and-hold vs 60-day momentum")
        self.debug("Momentum execution: explicit next-session MarketOnOpen orders")
        self.debug("Regime timing: today's return uses yesterday's regime label")
        self.debug("Subperiod 1: 2020-01-01 through 2022-12-31")
        self.debug("Subperiod 2: 2023-01-01 through the backtest end")
        self.debug(f"Total daily records: {len(self.daily_records)}")

        first_period_records = [
            record for record in self.daily_records
            if record["date"].year <= 2022
        ]
        second_period_records = [
            record for record in self.daily_records
            if record["date"].year >= 2023
        ]

        self.log_subperiod_summary("2020_2022", first_period_records)
        self.log_subperiod_summary("2023_2026", second_period_records)

    def log_subperiod_summary(self, period_label, period_records):
        classified_records = [
        ...
        exposure = (
            len(invested_records) / len(period_records)
            if period_records else 0
        )

        self.debug(f"SUBPERIOD {period_label}")
        self.debug(f"{period_label} total records: {len(period_records)}")
        self.debug(
            f"{period_label} fully classified records: "
            f"{len(classified_records)}"
        )
        self.debug(f"{period_label} momentum exposure: {exposure:.2%}")

        self.debug(f"{period_label} MOMENTUM OVERALL SUMMARY")
        self.log_summary(
            f"{period_label}_momentum_all_days",
            period_records,
            "actual_portfolio_return",
        )

        self.debug(f"{period_label} MOMENTUM VOLATILITY REGIME SUMMARY")
        ...
        self.debug(f"{period_label} MOMENTUM TREND REGIME SUMMARY")
        self.log_group_summary(
            classified_records,
            "trend_regime",
            "actual_portfolio_return",
        ...
        self.debug(f"{period_label} BUY-AND-HOLD OVERALL SUMMARY")
        self.log_summary(
            f"{period_label}_buy_and_hold_all_days",
            period_records,
            "qqq_return",
        )

        self.debug(f"{period_label} BUY-AND-HOLD VOLATILITY REGIME SUMMARY")
        self.log_group_summary(
            classified_records,
            "volatility_regime",
            "qqq_return",
        ...
        self.debug(f"{period_label} BUY-AND-HOLD TREND REGIME SUMMARY")
        self.log_group_summary(
            classified_records,
            "trend_regime",
            "qqq_return",
        ...
        momentum_cumulative = self.calculate_cumulative_return(
            period_records,
            "actual_portfolio_return",
        )
        buy_and_hold_cumulative = self.calculate_cumulative_return(
            period_records,
            "qqq_return",
        )
        return_difference = momentum_cumulative - buy_and_hold_cumulative

        self.debug(
            f"{period_label} DIRECT STRATEGY COMPARISON: "
            f"momentum_cum_return={momentum_cumulative:.2%}, "
            f"buy_and_hold_cum_return={buy_and_hold_cumulative:.2%}, "
            f"momentum_minus_buy_and_hold={return_difference:.2%}"
        )
