...
Purpose:
Test whether the strongest observed combined regime, low-volatility uptrend,
improves the simple QQQ daily momentum strategy when used as a trading filter.

Strategy rule:
Hold QQQ when trailing 60-day momentum is positive, prior volatility regime is
low volatility, and prior trend regime is uptrend. Otherwise hold cash.
...
class CrossStrategyRegimeAnalysis(QCAlgorithm):
    ...
    def initialize(self):
        ...
        # Baseline parameters chosen before seeing EXP-005 results.
    ...
    def on_data(self, data: Slice):
        ...
        self.apply_previous_signal_and_filters()
    ...
    def apply_previous_signal_and_filters(self):
        if self.previous_momentum_signal is None or self.previous_regime is None:
            return

        is_low_volatility = (
            self.previous_regime["volatility_regime"] == "low_volatility"
        )
        is_uptrend = self.previous_regime["trend_regime"] == "uptrend"

        should_hold_qqq = (
            self.previous_momentum_signal
            and is_low_volatility
            and is_uptrend
        )
    ...
    def on_end_of_algorithm(self):
        self.debug("EXP-005_QQQ_DAILY_MOMENTUM_LOW_VOL_UPTREND_FILTER")
        self.debug("Strategy: Hold QQQ when 60-day momentum is positive, volatility is low, and trend is up")
        self.debug("Regime timing: today's return uses yesterday's regime label")
        self.debug("Signal timing: today's position uses yesterday's momentum, volatility, and trend signals")
        ...
        self.debug(f"Momentum low-volatility uptrend exposure: {exposure:.2%}")

        self.debug("MOMENTUM LOW-VOL UPTREND FILTER STRATEGY OVERALL SUMMARY")
        self.log_summary("all_days", self.daily_records, "strategy_return")

        self.debug("MOMENTUM LOW-VOL UPTREND FILTER STRATEGY VOLATILITY REGIME SUMMARY")
        self.log_group_summary(classified_records, "volatility_regime", "strategy_return")

        self.debug("MOMENTUM LOW-VOL UPTREND FILTER STRATEGY TREND REGIME SUMMARY")
        self.log_group_summary(classified_records, "trend_regime", "strategy_return")

        self.debug("MOMENTUM LOW-VOL UPTREND FILTER STRATEGY COMBINED REGIME SUMMARY")
