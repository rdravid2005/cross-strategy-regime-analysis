...
Purpose:
Test a simple QQQ daily momentum strategy and summarize its daily strategy
returns across the same volatility and trend regimes used in EXP-001.

Strategy rule:
Hold QQQ when trailing 60-day momentum is positive. Otherwise hold cash.

Important timing rule:
Today's portfolio position is based on the momentum signal calculated from
prior data. Today's return is also assigned to the regime label known from
prior data. This avoids using today's close to decide today's exposure.
"""
...
class CrossStrategyRegimeAnalysis(QCAlgorithm):
    ...
    def initialize(self):
        ...
        # Baseline parameters chosen before seeing EXP-002 results.
    ...
    def on_data(self, data: Slice):
        ...
        # Trade using the signal that was already known before this bar.
        self.apply_previous_momentum_signal()
    ...
    def apply_previous_momentum_signal(self):
        if self.previous_momentum_signal is None:
            return

        if self.previous_momentum_signal:
    ...
    def on_end_of_algorithm(self):
        self.debug("EXP-002_QQQ_DAILY_MOMENTUM_REGIME_BASELINE")
        self.debug("Strategy: Hold QQQ when trailing 60-day momentum is positive")
